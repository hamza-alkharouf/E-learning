...
class Pager implements PagerInterface
{
	...
	public function newlinks(string $group = 'default', string $template = 'default_full',$data = [] ): string
	{
		$this->ensureGroup($group);

		return $this->newdisplayLinks($group, $template,$data);
	}
	...
	protected function newdisplayLinks(string $group, string $template,$data = [] ): string
	{
		if (! array_key_exists($template, $this->config->templates))
		{
			throw PagerException::forInvalidTemplate($template);
		...
		return $this->view->setVar('pager', $pager)->setVar('data', $data)
						->render($this->config->templates[$template]);
	}
	//--------------------------------------------------------------------
